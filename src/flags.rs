...
pub const OVERFLOW_BIT : u8 = 0b100_0000;
pub const NEGATIVE_BIT: u8 = 0b1000_0000;
...
pub fn clear_decimal(mut flags: &mut u8){
    clear_flag(&mut flags, DECIMAL_MODE_BIT);
}

pub fn clear_zero(mut flags: &mut u8){
    clear_flag(&mut flags, ZERO_BIT);
}

pub fn clear_negative(mut flags: &mut u8){
    clear_flag(&mut flags, NEGATIVE_BIT);
}
