...
pub fn zero_page(mut in_val : u8, operand: u8, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
    let mem_value = memory.read_mem_value(operand as u16);
    in_val = match_on_op(in_val, mem_value, op, status_flag);
    in_val as u8
}

pub fn zero_page_x(mut in_val : u8, x_val : u8, operand: u8, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
...
pub fn absolute(mut in_val : u8, operand: u16, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
...
pub fn absolute_reg(mut in_val : u8, reg : u8, operand: u16, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
    // little endian so swap around the operand bytes
...
pub fn indexed_indirect(mut in_val : u8, x_val : u8, operand: u8, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
    let addr = operand.wrapping_add(x_val);
...
pub fn indirect_indexed(mut in_val : u8, y_val : u8, operand: u8, memory: &mut RAM, status_flag: &mut u8, op : Operation) -> u8 {
