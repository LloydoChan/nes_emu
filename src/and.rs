...
pub fn and_zero_page(operand : u8, pc_reg : &mut u16, accumulator: &mut u8, status_flags: &mut u8, memory : &RAM, cycles_until_next : &mut u8){
    *accumulator = addressing::zero_page(*accumulator, operand, memory, status_flags, Operation::And);
    *pc_reg += 2;
    *cycles_until_next = 3;
}

pub fn and_zero_page_x(operand : u8, x_reg : u8, pc_reg : &mut u16, accumulator: &mut u8, status_flags: &mut u8, memory : &RAM, cycles_until_next : &mut u8){
...
pub fn and_indexed_indirect(operand : u8, x_val : u8, pc_reg : &mut u16, accumulator: &mut u8, status_flags: &mut u8, memory : &RAM, cycles_until_next : &mut u8){
